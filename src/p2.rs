...
impl Triangle {
    ...
    fn draw(&self, canvas: &sdl2::render::WindowCanvas) {
        ...
        let _ = canvas.filled_polygon(&vx, &vy, Color::RGB(255, 0, 0));
    }
}
...
fn main () -> Result<(), String> {
    ...
    let window = video_subsystem.window("Trabajo de práctica", SCREEN_WIDTH, SCREEN_HEIGHT).position_centered().build().map_err(|e| e.to_string())?;
